...
import { readableStreamToString, WorkdayPerformanceEnablementError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new WorkdayPerformanceEnablementError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new WorkdayPerformanceEnablementError(e, e.response?.data)
                }
                if (innerError instanceof WorkdayPerformanceEnablementError) {
                    // Got "WorkdayPerformanceEnablementError" from the above try block
                    throw innerError;
                }
